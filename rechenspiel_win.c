...
int main (int argc, char *argv[]) {
    ...
    if (!SetConsoleOutputCP(65001)) { // Setzen der Konsole auf UTF-8
        printf("Fehler beim Setzen der UTF-8-Locale.\n");
        return 1; // Programm mit Fehlercode beenden
    }
    if (!SetConsoleCP(65001)) { // Setzen der Eingabe auf UTF-8
        printf("Fehler beim Setzen der UTF-8-Locale.\n");
        return 1; // Programm mit Fehlercode beenden
    }
    setlocale(LC_ALL, "de_DE.UTF-8"); // Setzen der deutschen Locale für die korrekte Darstellung von Zahlen und Zeichen
